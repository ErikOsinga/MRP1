...
def find_orientationNN(i,fitsim,ra,dec, nn_ra, nn_dec, nn_dist,maj, s = (3/60.),plot=False,head=None,hdulist=None):
	'''	
	...
	data_array = extr_array(True,fitsim,ra,dec,nn_ra,nn_dec,nn_dist,maj,s=s
	...
	if True in (np.isnan(data_array)):
		if s < (2/60.):
			...
			try: 
				return find_orientationNN(i,fitsim,ra,dec,nn_ra,nn_dec,nn_dist,maj
	...
	(max_angle, err_orientations, min_orientation
		, max_orientation, classification, positions) = flux_along_line(data_array,radius,cutoff,hdulist)
