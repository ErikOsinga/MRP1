'''
Going to try to upgrade the scipy.spatial.ckdTree to the KDTree_plus 

to include the great-sphere distance when searching for Nearest Neighbours
'''
...
__all__ = ['minkowski_distance_p', 'minkowski_distance',
           'distance_matrix',
           'Rectangle', 'KDTree_plus']


def distanceOnSphere(RAs1, Decs1,x):
    """
...
def minkowski_distance_p(x, y, p=2):
    ...
    if p == np.inf:
        ...
        return distanceOnSphere(y[0][0],y[0][1],x)
    else:
...
class KDTree_plus(object):
    ...
    See Also
    --------
    cKDTree_plus : Implementation of `KDTree_plus` in Cython
    ...
    def __build(self, idx, maxes, mins):
        if len(idx) <= self.leafsize:
            return KDTree_plus.leafnode(idx)
        else:
            data = self.data[idx]
            ...
            greatermins[d] = split
            return KDTree_plus.innernode(d, split,
    ...
    def __query(self, x, k=1, eps=0, p=2, distance_upper_bound=np.inf):
        ...
        while q:
            ...
            if isinstance(node, KDTree_plus.leafnode):
                # brute-force
    ...
    def query(self, x, k=1, eps=0, p=2, distance_upper_bound=np.inf):
        ...
        >>> tree = spatial.KDTree_plus(list(zip(x.ravel(), y.ravel())))
        >>> tree.data
        array([[0, 2],
               [0, 3],
               [0, 4],
    ...
    def __query_ball_point(self, x, r, p=2., eps=0):
        ...
        def traverse_checking(node, rect):
            ...
            elif isinstance(node, KDTree_plus.leafnode):
    ...
    def query_ball_point(self, x, r, p=2., eps=0):
        ...
        substantial amounts of time by putting them in a KDTree_plus and using
        query_ball_tree.
        ...
        >>> tree = spatial.KDTree_plus(points)
        ...
        if x.shape[-1] != self.m:
            raise ValueError("Searching for a %d-dimensional point in a "
                             "%d-dimensional KDTree_plus" % (x.shape[-1], self.m))
        if len(x.shape) == 1:
    ...
    def query_ball_tree(self, other, r, p=2., eps=0):
        ...
        def traverse_checking(node1, rect1, node2, rect2):
            ...
            elif isinstance(node1, KDTree_plus.leafnode):
                if isinstance(node2, KDTree_plus.leafnode):
            ...
            elif isinstance(node2, KDTree_plus.leafnode):
        ...
        def traverse_no_checking(node1, node2):
            if isinstance(node1, KDTree_plus.leafnode):
                if isinstance(node2, KDTree_plus.leafnode):
                    for i in node1.idx:
    ...
    def query_pairs(self, r, p=2., eps=0):
        ...
        def traverse_checking(node1, rect1, node2, rect2):
            if rect1.min_distance_rectangle(rect2, p) > r/(1.+eps):
                ...
                # original KDTree_plus.query_pairs)
                if id(node1) != id(node2):
                    traverse_checking(node1.greater,greater1,node2.less,less2)
    ...
    def sparse_distance_matrix(self, other, max_distance, p=2.):
        ...
        Computes a distance matrix between two KDTree_pluss, leaving as zero
