...
def statistics(filename,tdata,redshift=False):
	...
	if parallel_transportbool:
		Sn_data = angular_dispersion_parallel_transport(tdata,n)
	else:
		Sn_data = angular_dispersion(tdata,n)

	Sn_mc = np.asarray(Sn_mc)
	av_Sn_mc = np.average(Sn_mc)
	sigma = np.std(Sn_mc)
	
	N = len(tdata)
	jain_sigma = (0.33/N)**0.5
	SL = 1 - norm.cdf(   (Sn_data - av_Sn_mc) / (sigma)   )

	print ('Sn_data: %f, Sn_mc: %f, Significance Level: %f' %(Sn_data,av_Sn_mc,SL))

	Result = Table()
	Result['Sn_data'] = Sn_data
	Result['Sn_mc'] = av_Sn_mc
	Result['SL'] = SL
	Result.write('./statistics_full_sample.fits',overwrite=True)
