...
def load_srclist(Mosaic,srclist):
	...
	return RA, DEC, Source_Name, Mosaic_ID, Total_flux, E_total_flux

def load_srclist2(Mosaic,srclist):
	"""
	Given the string Mosaic, loads all the S_Code, Maj, Min 
	from the source_list
	"""
	
	where = srclist['Mosaic_ID'] == Mosaic[:8]
	S_Code = srclist['S_Code'][where]
	Maj = srclist['Maj'][where]
	Min = srclist['Min'][where]

	return S_Code,Maj,Min


def setup_find_orientation_multiple_gaussians(file,SN=10,plot=False):
	'''
	...
	'''
	RA, DEC, Source_Name, Mosaic_ID, Total_flux, E_Total_flux = load_srclist(file,srclist)
	S_Code, Maj, Min = load_srclist2(file,srclist)
	...
	for i in multiple_gaussian_indices:
		if ( (Total_flux[i] / E_Total_flux[i]) > SN):
			if ( (Maj[i] - 1) < Min[i]):
				...
				r_index, r_orientation, r_err_orientation, r_amount_of_maxima, r_classification, r_lobe_ratio, r_position_angle = find_orientation(i,'useless',RA[i],DEC[i],Maj[i],Min[i],(3/60.),plot=plot,head=head,hdulist=hdulist)
	...
	if ( len(source_names) == 0 ):
		...
		F = open('../source_filtering/MG/'+file+'_parameters.txt','w') # // EDITED 
		...
		results.write('../source_filtering/MG/'+file+'_multiple_gaussians.fits',overwrite=True) # // EDITED 
		F = open('../source_filtering/MG/'+file+'_parameters.txt','w') # // EDITED 
		F.write('SN is ' +str(SN))
		F.write('\namount of multiple gaussian sources ' + str(len(source_names)))
		F.close()
...
for name in FieldNames:
	print name
	setup_find_orientation_multiple_gaussians(name,SN=10,plot=False)
 
# then cross-matching these catalogs to one single file
import cross_catalogs_MG
