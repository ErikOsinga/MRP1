...
from utils import load_in, deal_with_overlap, distanceOnSphere, tableMosaic_to_full
...
# First preprocess based on distance 
def check_distance(tdata):
	# First, check if we don't have double sources by imposing a 
	# minimum distance between to sources of 0.2 arcmin
	print ('Number of sources before distance check:',len(tdata))
	distances = NN_distance_final(tdata)
	sort = np.sort(distances)
	argsort = np.argsort(distances)
	remove = argsort[sort<0.2]
	# print (tdata['Source_Name'][remove])
	# print (np.asarray(distances)[remove])
	print ('Number of sources to remove:', len(remove)/2.0)
	del_rows = []
	for i in range(0,len(remove),2): #deleta half the sources (because duplicates)
		del_rows.append(i)
	tdata.remove_rows(del_rows)
	print ('Number of sources after distance check :',len(tdata))

	return tdata

# Then join the catalogs on component name 
def join_catalogs_componentname(tdata,Write=False):
	...
	print ('Actual amount of matches on components: %i' % len(result))
	...
	# Finally, write it out if you so want to
	if Write:
...
def components(tdata,tdata_NN):
	"""
	Investigate if the components match with my data
	by seeing how many component counts there are for every source
	
	tdata -- the table to investigate
	tdataNN -- only the NN sources of the table to investigate
			made by tdata_NN = tdata[np.invert(np.isnan(tdata['new_NN_RA']))]  
	...
	# print (indices)
	# source_name_2s of the multiple component sources
	multiple_comp_names = names[indices]
	print ('Number of all our sources in VA catalog with multiple components:',len(multiple_comp_names))

	result_only_NN = join(tdata_NN,comp,keys='Source_Name_2',uniq_col_name='{col_name}_{table_name}_3')
	namesNN, countsNN = np.unique(result_only_NN['Source_Name_2'],return_counts=True)
	indicesNN =  np.where(countsNN > 1)
	# source_name_2s of the NN multiple component sources
	multiple_comp_namesNN = namesNN[indicesNN]
	print ('Number of our NN sources in VA catalog with multiple components:',len(multiple_comp_namesNN))
...
if __name__ == '__main__':
	...
	# MG sources: use PA_1 (almost always same as LGZ_PA)
	...
	incorr_sourcenames, correct_sourcenames = components(VA_bs,VA_bs_NN)[2:4]
	...
	# for size calculated as NN dist:
	corr_table['size_thesis'] = corr_table['new_NN_distance(arcmin)']*60
	# physical size
	corr_table['physical_size'] = physical_size(corr_table['z_best'],corr_table['size'])
	# physical_size_thesis
	corr_table['physical_size_thesis'] = physical_size(corr_table['z_best'],corr_table['size_thesis'])
	...
	corr_table['power_thesis'] = power(corr_table['z_best'], corr_table['Total_flux_1']+corr_table['new_NN_Total_flux'])
	...
	notround_sources['size_thesis'] = notround_sources['new_NN_distance(arcmin)']*60
	# physical size
	notround_sources['physical_size'] = physical_size(notround_sources['z_best'],notround_sources['size'])
	# physical size_thesis
	notround_sources['physical_size_thesis'] = physical_size(notround_sources['z_best'],notround_sources['size_thesis'])
	...
	# not round sources power thesis: dont care about wrong match use Total flux 1 + NN total flux
	notround_sources['power_thesis'] = power(notround_sources['z_best'],notround_sources['Total_flux_1']+notround_sources['new_NN_Total_flux'])
	...
	# size_thesis is then the same
	round_sources['size_thesis'] = round_sources['size']
	# physical size
	round_sources['physical_size'] = physical_size(round_sources['z_best'],round_sources['size'])
	# physical size thesis also
	round_sources['physical_size_thesis'] = physical_size(round_sources['z_best'],round_sources['size'])
	...
	# same story
	round_sources['power_thesis'] = round_sources['power']
	...
	VA_bs_NN.write('../value_added_selection_NN.fits',overwrite=True) # just assume that its correctly matched
