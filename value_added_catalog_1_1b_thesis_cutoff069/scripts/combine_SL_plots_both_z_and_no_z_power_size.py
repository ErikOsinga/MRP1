...
def plot_additional_info(tdata,filename,redshift):
	...
	if 'size' in filename and not redshift: # Only have to plot this once
		plt.plot((0,0),(0,0),c='white',label='Min size %.2E kpc'%Decimal(np.min(tdata['physical_size'])))
		plt.plot((0,0),(0,0),c='white',label='Max size %.2E kpc'%Decimal(np.max(tdata['physical_size'])))
	if 'power' in filename and not redshift: # Only have to plot this once
		plt.plot((0,0),(0,0),c='white',label='Min power %.2E '%Decimal(np.min(tdata['power'])) + 'mJy*kpc^2')
		plt.plot((0,0),(0,0),c='white',label='Max power %.2E '%Decimal(np.max(tdata['power'])) + 'mJy*kpc^2')
...
def merge_figures_z_and_no_z(load=True):
	...
	def produce_subset_figures(starting_i,filename,load=True):
		...
		for key in sorted(fluxbins):
			...
			filename = filename_original_no_z + 'power%s'%key
			...
			filename = filename_original_redshift + 'power%s'%key
